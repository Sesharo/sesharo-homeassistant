...
from custom_components.sesharo.discovery import (
    discover_candidates,
    slugify_signal,
    suggest_mapping,
)
...
def test_suggest_binary_sensor_is_event_no_unit():
    state = FakeState(
        "binary_sensor.front_door", "on", device_class="door", friendly_name="Front Door"
    )
...
def test_discover_skips_already_mapped_entities():
    cands = discover_candidates(_states(), {"sensor.washer_cycles"}, set(), presets_enabled=True)
    entities = {c[CONF_CUSTOM_ENTITY] for c in cands}
