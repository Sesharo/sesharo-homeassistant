...
| File | Role |
|---|---|
| `const.py` | `DOMAIN`, config keys, the preset catalog (`PRESET_METRICS` by `device_class`, `PRESET_EVENTS`, `PERSON_EVENT_CATEGORY`), `PRESET_CATALOG` (panel row metadata: label + MDI icon per preset), per-preset opt-out key `CONF_PRESET_DISABLED`, custom-mapping `CONF_CUSTOM_TARGET_UNIT`, panel constants, defaults. |
